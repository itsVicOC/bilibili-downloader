...
一款支持 GUI 和 CLI 的 B 站内容归档工具。可以解析视频、番剧、合集、系列与收藏夹，管理可跨重启恢复的任务，并将当前账号已获完整播放权限的未加密媒体流及字幕、弹幕、封面和元数据保存到本地。
...
- 支持 BV、AV、番剧 `ep/ss/md`、Bilibili 完整 URL、b23.tv 短链、UP 主合集/系列和收藏夹。
...
- 支持普通视频与番剧的独立安全目录模板；番剧可按系列、季度、章节和集号归档。
- 支持 SESSDATA 或完整 Cookie 字符串登录；只保留白名单字段并保存到系统凭据库。
...
2. 粘贴 BV/AV/ep 号、视频/番剧单集 URL 或 b23.tv 短链，点击“开始解析”。
3. 选择画质、编码、视频/音频模式及归档附加项，加入任务中心。
4. 合集、收藏夹或番剧 `ss/md` 页面使用“批量导入”；番剧默认勾选正片，PV、花絮和特别篇可手动选择。
5. 首次真正加入下载任务时阅读版权提示；确认后才会创建任务。
6. 在“下载设置”中调整输出目录、普通视频/番剧目录模板和最大并发数。
...
| [版权与合规说明](docs/COPYRIGHT.md) | 授权边界、禁止用途与权利人反馈 |
...
- 应用不会把登录凭据发送到本项目维护者、自建服务器或媒体 CDN；Cookie 仅发送给受信任的 Bilibili API/网页域名。
- 登录 Cookie 包只保留白名单字段并存储在系统凭据库。凭据库不可用时仅在当前进程内存中保留，重启后需重新登录。
- 本项目与哔哩哔哩（Bilibili）无隶属、授权或背书关系。
- 账号可观看不自动等于拥有永久复制、传播或商业使用权。请阅读[版权与合规说明](docs/COPYRIGHT.md)，并遵守当地法律、平台服务条款和权利人许可。
